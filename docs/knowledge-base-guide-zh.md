...
webb.org (69 页)     ──►  data/scraped/*.json  ──►  ChromaDB
  通过 scraper.py           (纯文本)                  776 个 chunks
                                                      768 维向量
...
1. **语料库太小** — 只有 776 个分块，检索后得到 20-35 个候选，生成模型（Sonnet）全部读完。没有必要进一步筛选。Reranking 的价值在于从 1,000+ 个候选中选出 10 个；我们没有这个问题。
