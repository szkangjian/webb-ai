...
| 方案 | 工作原理 | 权衡 |
|------|---------|------|
| **LLM 两轮检索** | 初始检索后，让 Haiku 判断："这个问题还需要参考哪些相关政策？"然后进行第二轮检索。 | +1-2 秒延迟，+$0.001/查询，但能自动发现交叉引用 |
| **索引时打标签** | 建索引时给每个 chunk 标注话题标签（纪律、住宿、学术等）。查询时除语义搜索外按话题检索。 | 需要重建索引，但无运行时成本 |
| **知识图谱** | 构建政策关系图（如 CBO → 影响 → 请假资格）。查询时遍历图谱。 | 最稳健，但开发工作量大 |
